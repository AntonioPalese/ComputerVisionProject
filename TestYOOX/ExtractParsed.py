...
def make(Fields, category, part):
    ...
    for i in range(data.shape[0]):
        ...
        parsed_img = np.delete(parsed_img, 3, 2)  # l' immagine parsata ha 4 canali (1 inutile) elimino il 3
        #target_img = np.delete(target_img, 3, 2)

        parsed_img = parsed_img * 255  # l' immagine parsata è compresa tra 0 e 1 riporto i valori tra 0 e 255
        ...
        # printitem = item.numpy().copy()
        # printitem = printitem.astype(np.uint8)
        # plt.imshow(printitem)
        # plt.show()
        # plt.imshow(mask)
        # plt.show()
