...
def parse_item(label : str, parsed : numpy.ndarray, img : numpy.ndarray, palette : dict):
    ...
    :param img: Immagine originale da cui estrarre la segmentazione
    :param palette: La palette di colori del segmentatore

    :return img, mask_item : una coppia contenente l'immagine e la maschera dell'item
    """
    ...
    # item = torch.where(parsed == t, parsed, 1.)
    # return  item
...
def load_palette(palette_file : str,dump = False):
    '''
    :param palette_file: il file della palette
    :param dump: booleano per dumpare il dizionario della palette in un file json
    :return: il dizionario della palette
    '''
